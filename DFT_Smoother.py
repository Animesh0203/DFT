...
# 8. Compile DFT Images into GIF with smoother animation
gif_path = 'dft_animation_smooth.gif'
with imageio.get_writer(gif_path, mode='I', duration=0.1) as writer:
    for i in range(len(dft_images)):
        if i < len(dft_images) - 1:
            current_image = imageio.imread(dft_images[i])
            next_image = imageio.imread(dft_images[i + 1])
            for j in range(5):  # Add 5 intermediate frames between consecutive frames
                blended_frame = (current_image * (4 - j) + next_image * (j + 1)) / 5
                writer.append_data(blended_frame.astype(np.uint8))
        else:
            writer.append_data(current_image)  # Append the last frame
